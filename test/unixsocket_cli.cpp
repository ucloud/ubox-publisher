...
int main(int argc, char** argv) {
    ...
    memcpy(srv_addr.sun_path, argv[1],strlen(argv[1]));
    srv_addr_len = sizeof(srv_addr.sun_family) + strlen(argv[1]);
    ...
    while(fgets(buff+8, 8192, stdin) != NULL) {
       ...
       *(int*)(buff+4) = ntohl(stringSize);
        ...
        if (ret == -1) {
            fprintf(stderr, "send error %s, %d, err(%d)%s\n", srv_addr.sun_path, srv_addr_len, errno, strerror(errno));
            break;
       }
