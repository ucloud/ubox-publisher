...
UnixDomainService::UnixDomainService(ControlHandler& handler): handler(handler) {
    srvFD = -1;
    opened = false;
...
int UnixDomainService::Open(const char *path)  {
    ...
    tlog(TLOG_INFO,  "%s, unix domain server open\n", __FUNCTION__);
    return 0;
}
...
void UnixDomainService::Wait() {
    ...
    while (true) {
        dataSize = recvfrom(srvFD, buf, MESSAGE_MAX_SIZE, 0, (struct sockaddr *)&fromAddr, &fromLength);
        //tlog(TLOG_INFO,  "unix domain socket read data %d\n", dataSize);
        if (dataSize <= 0) {
            tlog(TLOG_INFO,  "read data error: %d\n", dataSize);
            break;
        }
        // unpacket data
        cJSON* json = UnPacket(buf, dataSize);
        if (json == NULL) {
            sendSize = PacketErrorMsg(buf, MESSAGE_MAX_SIZE, RETCODE_TYPE_WRONG_DATA, "can not parse data");
            returnSize = sendto(srvFD, buf, sendSize, 0, (struct sockaddr *)&fromAddr, fromLength);
            if (returnSize < 0) {
                tlog(TLOG_INFO,  "send data error: %d from %s\n", returnSize, fromAddr.sun_path+1);
                break;
            }
            tlog(TLOG_INFO,  "send data: %s\n", "can not parse data");
            continue;
        }
        ...
        if (!cJSON_IsString(jsonCommand)) {
            ...
            sendSize = PacketErrorMsg(buf, MESSAGE_MAX_SIZE, RETCODE_TYPE_WRONG_DATA, "data no action");
            returnSize = sendto(srvFD, buf, sendSize, 0, (struct sockaddr *)&fromAddr, fromLength);
            if (returnSize < 0) {
                tlog(TLOG_INFO,  "send data error: %d\n", returnSize);
            }
            tlog(TLOG_INFO,  "send data: %s\n", "data no action");
            continue;
        }
        ...
        if (it == funcMap.end()) {
            ...
            sendSize = PacketErrorMsg(buf, MESSAGE_MAX_SIZE, RETCODE_TYPE_WRONG_DATA, "unknown action");
        } else {
            // parse other data
            ...
            if (jsonReturn != NULL) {
                ...
                sendSize = PacketErrorMsg(buf, MESSAGE_MAX_SIZE, RETCODE_TYPE_UNKNOWN_ERROR, "unknown error");
            }
        }
...
int UnixDomainService::Close() {
    tlog(TLOG_INFO,  "%s, unix domain server close\n", __FUNCTION__);
    if (srvFD != -1) {
...
cJSON* UnixDomainService::SetBitrate_Parse(cJSON* jsonRequest) {
    ...
    if (jsonBitrate->valueint > 100000 || jsonBitrate->valueint < 1) {
        return GetJsonByCode(RETCODE_TYPE_VALUE_INVALID, "bitrate value is invalid");
    }
...
cJSON* UnixDomainService::PushStream_Parse(cJSON * jsonRequest) {
    tlog(TLOG_INFO,  "%s, push stream:\"%s\"\n", __FUNCTION__, cJSON_Print(jsonRequest));
...
cJSON* UnixDomainService::CloseStream_Parse(cJSON * jsonRequest) {
    tlog(TLOG_INFO,  "%s, close stream:\"%s\"\n", __FUNCTION__, cJSON_Print(jsonRequest));
    ...
    if (ret != 0) {
        tlog(TLOG_INFO,  "stop stream failed, device(%s)\n", jsonDevice->valuestring);
...
cJSON*  UnixDomainService::UnPacket(unsigned char *buf, int size) {
    if (size < MESSAGE_HEADER_SIZE) {
        tlog(TLOG_INFO, "wrong data, data size is %d\n", size);
        return NULL;
    }
    if (buf[0] != 0x20 ||
    buf[1] != 0x21 ||
    buf[2] != 0x09 ||
    buf[3] != 0x01 ) {
        tlog(TLOG_INFO, "wrong data, data header is %02x %02x %02x %02x\n", buf[0], buf[1],buf[2],buf[3]);
        return NULL;
    }
    ...
    cJSON* json = cJSON_ParseWithLength((const char*)(buf+MESSAGE_HEADER_SIZE),  dataSize);
...
int UnixDomainService::Packet(unsigned char *buf, int bufSize, cJSON *jsonData) {
...
int UnixDomainService::PacketErrorMsg(unsigned  char* buf, int bufSize , int retCode, const char* msg) {
    snprintf((char*)(buf+MESSAGE_HEADER_SIZE), bufSize-MESSAGE_HEADER_SIZE, "{\"RetCode\":%d,\"Message\":\"%s\"}", retCode, msg);
